...
3. **Create Task List** _(skip if Phase 0 already built one, or if Phase 0 routed as Trivial)_
   - Use the platform's task tracking tool (`TaskCreate`/`TaskUpdate`/`TaskList` in Codex, `update_plan` in Codex, or the equivalent on other harnesses) to break the plan into actionable tasks
...
4. **Choose Execution Strategy**
   ...
   **Subagent isolation** — give each parallel subagent its own working tree:
   - **Codex (`Agent` tool):** pass `isolation: "worktree"` and `run_in_background: true`. The harness creates a per-subagent worktree under `.Codex/worktrees/agent-<id>` on its own branch. Verify `.Codex/worktrees/` is gitignored before relying on this.
