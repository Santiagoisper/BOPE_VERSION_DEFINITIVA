---
name: ce-sessions
description: "Search and ask questions about coding agent session history across Codex, Codex, and Cursor. Use when asking what was worked on, what was tried before, how a problem was investigated across sessions, what happened recently, or any question about past agent sessions. Also use when the user references prior sessions, previous attempts, or past investigations — even without saying 'sessions' explicitly."
---

# /ce-sessions

Search session history across Codex, Codex, and Cursor and synthesize findings about what was worked on, tried, decided, or learned in prior sessions.
...
- **Never include thinking or reasoning block content.** Codex thinking blocks are internal reasoning; Codex reasoning blocks are encrypted. Neither is actionable.
...
Codex retains session history for ~30 days by default. Wider windows may find nothing on Codex unless the user has extended retention.
...
To narrow the platform set, add `--platform Codex`, `--platform codex`, or `--platform cursor` to the `discover-sessions.sh` invocation. Default to all three.

### Step 3 — Filter and rank

Apply these filters in order to pick the sessions worth deep-diving:

1. **Branch filter (Codex only).** Keep sessions where `branch == dispatch_branch` exactly, or where the branch name contains a keyword from the question's topic (e.g., a question about "auth middleware" matches branches `feat/auth-fix`, `chore/auth-refactor`). Codex sessions don't carry `gitBranch` — skip this filter for them.
...
Dispatch the `ce-session-historian` subagent via the platform's subagent primitive (`Agent` in Codex, `spawn_agent` in Codex, `subagent` in Pi via the `pi-subagents` extension). Omit the `mode` parameter so the user's configured permission settings apply. Run on the mid-tier model (e.g., `model: "sonnet"` in Codex) — the synthesizer doesn't need frontier reasoning.
...
- `sessions` — an array of objects, one per extracted session, each with:
  ...
  - `platform` — `Codex`, `codex`, or `cursor`
  - `branch` — git branch when present (Codex only)
...
1. /tmp/ce-sessions-XXXX/abc123.skeleton.txt
   platform=Codex branch=feat/auth-fix ts=2026-05-01
...
```
**Sessions searched**: [count] ([N] Codex, [N] Codex, [N] Cursor) | [date range]
```
