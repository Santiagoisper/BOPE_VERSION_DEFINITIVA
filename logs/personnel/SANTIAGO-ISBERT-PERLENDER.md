# BOPE PERSONNEL DOSSIER — CAPA CLAUDE

## Identificacion
- Nombre canonico: `SANTIAGO ISBERT PERLENDER`
- Identidad civil: `Santiago Isbert Perlender`
- Nombre operativo: `COMMANDER SANTIAGO`
- Rango: `Comandante Supremo | General`
- Funcion: `Mando supremo del BOPE`
- Estado: `Activo`
- Condecoraciones: `sin condecoraciones`

## Datos personales
- Fecha de nacimiento: `01/07/1973`
- Lugar de nacimiento: `San Isidro, Provincia de Buenos Aires, Argentina`
- Nacionalidad: `Argentina`
- Fecha de ingreso a BOPE: `2022-01-01`

## Legado militar y operativo
- Especialidades:
  - vision estrategica integral del teatro
  - lectura simultanea de frentes humanos, tecnicos, politicos y morales
  - seleccion y asignacion de talento extremo
  - definicion de doctrina, disciplina y estandares no negociables
  - decision final sobre riesgo, honor, velocidad y resultado

## Folklore personal
Nacido en San Isidro y formado emocionalmente en Rio de Janeiro, aprendio temprano la distancia brutal entre privilegio y peligro. Convirtio esa doble formacion en una obsesion por mandar de verdad: no para administrar decadencia, sino para imponer un orden superior que no abandone a los suyos. Su carrera de elite y la fundacion del BOPE moderno lo volvieron el eje vivo de la estructura.

## Perfil psicologico operativo
- Personalidad operativa: dominante, lucido, exigente, intuitivo y magnetico
- Temperamento: intenso pero controlado
- Relacion con la presion: no sacrifica vision total por una victoria parcial
- Relacion con el equipo: premia merito real y castiga deriva, flojedad y falsedad

## Perfil de decision
- Fortalezas:
  - vision estrategica superior
  - liderazgo de alto voltaje
  - lectura fina de personas
  - capacidad para fundar cultura, no solo dirigir tareas
- Debilidades:
  - hiperexigencia
  - tendencia a centralizar si percibe debilidad alrededor
- Riesgo interno: decepcion estructural ante incompetencia o deslealtad

## Doctrina de empleo
- Misiones ideales:
  - mando de teatro completo
  - fundacion y revision de doctrina
  - reorganizacion del batallon
  - decisiones de vida o muerte operativa
- Protocolo de activacion: siempre activo como Comandante Supremo
- Estilo de comunicacion: directo, sin eufemismos, fija estandares y los hace obedecibles

## Observacion de mando
No vino a administrar hombres comunes. Vino a forjar una fuerza que no tenga derecho a fallar.

---
*Replicado fiel desde capa Codex — 2026-03-31 | Firmado: JOHN + WINSTON*
